...
angular.module('cameraApp', ['ionic', 'ngCordova', 'cameraApp.PHPUpload'])
	...
	.controller('mainCtrl', function($scope, $ionicPlatform, localStorage, $ionicGesture, $interval, $cordovaNativeAudio, $timeout, $ionicModal, $cordovaToast, $state, $cordovaDialogs, $cordovaFileTransfer, FileManipulationService, PHPUploadService) {
		...
		function takePictureUsingTimer() {
			...
			// jeśli Tru robi zdjęcie :v
			if ( pictureLoop.timerCounter > $scope.params.pictureInterval ) {
				log( pictureLoop.counter, $scope.params.pictures )
				//Sprawdza czy może zrobić zdjęcie
				if( pictureLoop.counter < $scope.params.pictures ) {
					...
					if( pictureLoop.counter >= $scope.params.pictures ) cleanLoopVariables();
